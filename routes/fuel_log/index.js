...
fuelLog.post('/delete_log/', (req, res) => {
    console.log("Deleting new log")


    const idLog = req.body.id_log
    
    const _action = 'delete_log'

    const queryString = "CALL `heroku_cd69aac1f1eff94`.`fuelLogs_data`(?, '','','','','','', '', '', '', '', '','','',?);"


    bd_connection.query(queryString,[idLog,_action], (err, rows)=>{
        if(err){
            console.log(err)
            console.log("Here: Fuel log deleting error")
            return
        }
        console.log("Success deleting log")
    })
    res.end()
